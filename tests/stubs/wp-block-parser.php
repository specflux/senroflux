<?php
// phpcs:disable -- Verbatim WordPress core source; not this project's code to restyle.
/**
 * WordPress core's REAL block parser, vendored for the test suite.
 *
 * TARGET REPO PATH: tests/stubs/wp-block-parser.php
 *
 * Copied verbatim (minus core's trailing `require_once` lines, which this file
 * satisfies by concatenation) from WordPress:
 ...
 *
 * WordPress is free software released under the GNU General Public License
 * version 2 or later; this plugin ships under the same licence (see LICENSE),
 * so the vendored copy stays licence-compatible.
 *
 * WHY: the pages pack's whole write contract is
 * `serialize_blocks( parse_blocks( $content ) ) === $content` plus a walk over
 * `innerBlocks` / `innerHTML`. A hand-rolled stub would make those tests true
 * by construction. Only the real parser — with its `innerContent` null markers
 * and its freeform (`blockName === null`) handling — can fail them.
 *
 * @package SenroFlux
 */
